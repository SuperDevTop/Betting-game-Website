...
if($mr_u == null){
    header("location:/stage/withdraw_pending_history");
}
...
if(isset($_POST['saqNome'])){
    ...
    //echo (float)$saldo .">=". (float)$sv ;
    
    if((float)$sv >= 50){
        if((float)$saldo >= (float)$sv){
            if($_POST['pixoubanco'] == "dadosPix"){
                ...
                $sql = "INSERT INTO withdraws (user_id,full_name,cpf,bank,agency,account_type,account,requested_amount,status,created_at,updated_at) VALUES ";
                $sql.= "($id_user,'$sn','$sc','$sb',$sa,'$stp',$scc,'$sv','Pending','$dt','$dt')";
                //echo $sql;
            }
            if($conn->consultar($sql)){
                $novoSaldo = (float)$saldo - (float)$sv;
                $conn->consultar("UPDATE users SET balance = '$novoSaldo' WHERE email = '$mr_u'");
                header("location:/stage/withdraw_pending_history");
            }
        }else{
...
<head>
	...
	<title>Million Rocket - Solicitação de Saque</title>
...
<body class="forms">
    ...
    <span class="frm_box_titulo">
        ...
        <h2 class="tituloCLR">Formulário de Solicitação de saque.</h2>
    </span>
    <form class="frmCLR" method="post" action="">
        <p style="color:#f44336;">⚠️️ Os dados bancarios devem ser o mesmo do cadastro do jogador. Caso os dados bancários não sejam do Jogador, o valor será estornado para apostas.</p>
